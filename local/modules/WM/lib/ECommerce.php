 ...
 * @package WM
 */
class ECommerce extends StaticInstance
{
    /**
     * @var loaded order
     */
    ...
    public function jsonResult($return = false, $needUpdate = false)
    {
        ...
        $ret .= 'dataLayer.push(' . $this->getJson($this->getGoogleCode($needUpdate)) . ');' . PHP_EOL;
        $ret .= 'dataLayer.push(' . $this->getJson($this->getYandexCode($needUpdate)) . ');' . PHP_EOL;
        $ret .= '</script>' . PHP_EOL;
        if($return)
            return $ret;
        echo $ret;
